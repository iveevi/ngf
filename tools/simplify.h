...
struct SymetricMatrix {
	...
	SymetricMatrix(double c = 0) {
        ...
        double det(int a11, int a12, int a13, int a21, int a22, int a23,
        ...
        const SymetricMatrix operator+(const SymetricMatrix &n) const {
                return SymetricMatrix(m[0] + n[0], m[1] + n[1], m[2] + n[2],
...
void update_mesh(int iteration)
{
	...
	loopi(0,triangles.size())
	{
		...
		{
			printf("Triangle %d has negative vertex index %d %d %d\n", i, t.v[0], t.v[1], t.v[2]);
			exit(1);
		}
...
void load_obj(const char* filename, bool process_uv=false)
{
	...
	bool has_uv = false;
        ...
        // Fix negative triangle indices
	for (int i = 0; i < triangles.size(); i++) {
		for (int j = 0; j < 3; j++) {
			if (triangles[i].v[j] < 0)
				triangles[i].v[j] += vertices.size() + 1;
		}
	}
        ...
        loopi(0, triangles.size()) if (!triangles[i].deleted) {
                ...
                if (has_uv) {
                        fprintf(file, "f %d/%d %d/%d %d/%d\n",
                                ...
                                triangles[i].v[1] + 1, uv + 1,
                                triangles[i].v[2] + 1, uv + 2);
                        uv += 3;
                } else {
